...
class GetRasterPixelValue:
    ...
    def __init__(self, layer: qgis.core.QgsRasterLayer):
    ...
    def getIndex(self, pos: qgis.core.QgsPointXY):
    ...
    def getValueFromPos(self, pos: qgis.core.QgsPointXY):
        ident = self.__provider.identify(pos, qgis.core.QgsRaster.IdentifyFormatValue )
    ...
    def getValueFromIdx(self, idxX: int, idxY: int):
        ...
        block = self.__provider.block(1, self.__extent, self.__width, self.__height)
        return block.value(idxY, idxX)


    def getValueInterpolation(self, pos: qgis.core.QgsPointXY):
        ...
        # このあたりの距離は厳密ではないです。tmp1とtmp2の位置を使うべき!!
    ...
    def __getXYZ__(self, idxX: int, idxY: int):
        ...
        block = self.__provider.block(1, self.__extent, self.__width, self.__height)
        z = block.value(idxY, idxX)

        return qgis.core.QgsPoint(x, y, z)